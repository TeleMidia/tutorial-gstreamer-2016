...
criados estendendo a classe base \C{GstElement}.  Também é possível encapsular
...
começar com um exemplo simples, que ao receber algum dado no seu \en{sink pad}
irá apenas imprimir o tamanho do buffer recebido e retransmitir esses dados em
seu \en{source pad}.  Iremos chamar esse novo elemento de ``myfilter''.  A
...
(\C{myfilter.h} e \C{myfilter.c}) e será detalhado a seguir.
...
definem macros padrões que usam o \en{framework} GObject para checagem e
...
possível, por exemplo, carregar o elemento ``myfilter'' que desenvolvemos em
...
filtros.  O nosso próximo exemplo---elemento \en{myvideofilter}---irá usar a
classe \C{GstVideoFilter} para exemplificar como é possível criarmos um
...
\en{myvideofilter} também está dividido em dois arquivos, \C{myvideofilter.h}
e \C{myvideofilter.c}.
...
anterior as principais diferenças são: as capacidades dos \en{pads}, a função
de transformação e o fato de que neste plugin também definimos as três
propriedades que permitem ao usuário controlar quais os canais de cores que
serão passados para o \en{src pad}.
...
@\$@ gst-launch-1.0 --gst-plugin-path=. filesrc location=bunny.ogg ! \
     ...
     video/x-raw,width=720 ! myvideofilter use-r ! ximagesink
\end{lstlisting}
