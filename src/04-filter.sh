case "$2" in
  ...
  1) af='pitch pitch=.5';                         vf='dicetv';;
  2) af='audioecho delay=1000000000 intensity=1'; vf='shagadelictv';;
...
if test -z "$1" || test -z "$vf" || test -z "$af" ; then
  echo >&2 "usage: ${0##*/} URI [0-4]"; exit 1
fi
gst-launch uridecodebin uri="$1" name=decbin\
