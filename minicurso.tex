...
\title{Programando aplicações multimídia no GStreamer~\rg{Prefiro o título
anterior: ``Desenvolvimento de aplicações multimídia usando o GStreamer''}}
\author{Guilherme F.~Lima, Rodrigo C.\,M.~Santos e Roberto
G.\,~de~A.~Azevedo~\rg{Esta ordem tem que ser a ordem dos autores. Estou bem no
último lugar, mas acho que é bom vocês converseram e decidirem quem será o
primeiro autor (e terá o maior trabalho de escrita).}}
...
um \emph{player} de vídeo interativo completo, com completo suporte às
...
Precisamos apenas de um projetor multimídia e de uma rede sem fio\rg{Com
