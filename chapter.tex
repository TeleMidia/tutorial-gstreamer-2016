...
indicando que o ``oggdemux'' não possui \en{sink pads} conectadas e o
controle é devolvido à \en{thread} da aplicação que termina o programa.

Um detalhe sutil mas importante no código da Listagem~\ref{lst:ogg} é que a
\en{callback} \C{pad_added_cb} pode ser disparada numa \en{thread} diferente
da \en{thread} da aplicação.  Nesse caso, caso haja dados compartilhados
entre as \en{threads} é preciso sincronizar o acesso a esses dados usando
\en{mutexes} e semáforos (tipos \C{GMutex} e~\C{GCond} da GLib).  Na
Listagem~\ref{lst:ogg} não precisamos fazer isso porque o único dado
compartilhado entre as \en{threads} da \en{callback} e da aplicação é o
elemento demultiplexador ``oggdemux'' e, indiretamente, o \en{pipeline}, e
porque as que chamadas que usamos na \en{callback} para manipular esses
dados, \C{gst_element_$\ast$} e~\C{gst_bin_$\ast$}, já são internamente
protegidas por \en{mutexes}.  Além disso, no intervalo em que as
\en{threads} da \en{callback} e da aplicação estão de fato concorrendo,
...
%  LocalWords:  vorbis video x-theora theora
