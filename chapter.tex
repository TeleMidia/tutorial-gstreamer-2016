...
(\en{pause}, \en{seek}, \en{fast-foward} e~\en{rewind}) e implementamos o
suporte a cada uma delas no exemplo.  Na Seção~\ref{sec:plugins}
apresentamos a arquitetura de \en{plugins} do GStreamer e mostramos como
...
Na Seção~\ref{sec:hello} utilizamos o elemento ``playbin'' para tocar um vídeo
Ogg no GStreamer.  Nesta seção, nosso objetivo é construir a mesma aplicação
mas dessa vez sem usar o elemento ``playbin''.  Usando apenas elementos
básicos (\en{sources}, demultiplexadores, decodificadores e \en{sinks})
vamos construir um \en{pipeline} semelhante ao da
...
e, finalmente, o \en{pipeline} transiciona de \en{paused} para \en{playing}.
Essa última transição completa a chamada e devolve o controle à função
\C{main}.  Nesse ponto, a \en{thread} da aplicação está prestes a executar a
asserção da linha~26, que aborta o programa em caso de falha da chamada
anterior, e as \en{streaming threads} já começaram a operar o \en{pipeline},
...
chamada linha~14 obtém as \en{caps} (\C{GstCaps}) associadas à \en{sometimes
  pad} criada (parâmetro \C{srcpad}) e a chamada da linha~15 obtém o nome do
tipo (\en{mime type}) do fluxo que escoará através da \en{pad}.  Nesse
ponto, há três possibilidades:
\begin{enumerate}
\item Se o tipo é Vorbis (``audio/x-vorbis'') e uma \en{pad} Vorbis ainda
  ...
  (linha~26), obtém a \en{sink pad} do primeiro elemento da série,
  ``vorbisdec'', (linha~27), e marca a \en{flag} \C{has_vorbis_pad}
  (linha~28) para indicar que uma \en{pad} Vorbis foi encontrada.
\item Se o tipo é Theora (``video/x-theora'') e uma \en{pad} Theora ainda
  ...
  do \en{pipeline}, conecta-os em série, obtém a \en{source pad} do
...
a \en{sink pad} do elemento ``vorbisdec'' se \C{srcpad} é Vorbis ou do
elemento ``queue'' se \C{srcpad} é Theora.  Finalmente, a chamada
\C{gst_pad_link} (linha~48) conecta a \C{srcpad} encontrada à \C{sinkpad} da
série apropriada, e as chamadas seguintes (linhas~50--53) liberam as
...
Se tudo correr bem, quando iniciado, o programa da Listagem~\ref{lst:ogg}
demultiplexa o arquivo Ogg fornecido e reproduz os primeiros subfluxos
Vorbis e Theora que encontrar.  Caso não haja subfluxos Vorbis ou Theora, o
programa termina imediatamente---uma mensagem de error é postada no \en{bus}
indicando que o ``oggdemux'' não possui \en{sink pads} conectadas e, em
seguida, o controle volta à \en{thread} da aplicação que termina o programa.

Há um detalhe sutil mas importante no código da Listagem~\ref{lst:ogg}: a
\en{callback} \C{pad_added_cb} executa numa \en{thread} diferente da
\en{thread} da aplicação.  Consequentemente, caso haja dados compartilhados
entre as \en{threads}, para evitar condições de corrida, é necessário
sincronizar o acesso a esses dados usando \en{mutexes} e semáforos (tipos
\C{GMutex} e~\C{GCond} da GLib).  Na Listagem~\ref{lst:ogg} não precisamos
fazer isso porque as que chamadas \C{gst_element_$\ast$}
e~\C{gst_bin_$\ast$} que usamos na \en{callback} para manipular o elemento
``pipeline'' (de fato, o único dado compartilhado entre as \en{threads} da
\en{callback} e da aplicação) são protegidas internamente por \en{mutexes}.
Além disso, no intervalo em que as \en{threads} da \en{callback} e da
aplicação estão de fato concorrendo, linhas~77--80, a \en{thread} da
aplicação não modifica o \en{pipeline}.
...
\en{playing}).  O programa \en{gst-launch}, instalado pelo pacote
\en{gstreamer}, permite construir \en{pipelines} desse tipo diretamente na
linha de comando.  Por exemplo, os três comandos seguintes constroem e
iniciam \en{pipelines} idênticos aos das Listagens~\ref{lst:hello},
...
Usando o \en{gst-launch} é relativamente simples construir uma versão geral
do \en{pipeline} da Listagem~\ref{lst:ogg}, isto é, construir um
...
reproduz os fluxos resultantes num ambiente arbitrário (não apenas
...
\end{lstlisting}
O \en{pipeline} anterior possui quatro elementos: ``uridecodebin'',
...
O elemento ``audioconvert'' é um conversor de formato de áudio PCM.  E os
elementos ``audioaudiosink'' e ``autovideosink'' são \en{sinks} abstratos de
áudio e vídeo que detectam e usam os \en{sinks} concretos mais adequados
para um dado ambiente.  Por exemplo, no nosso ambiente GNU/Linux os
elementos ``autoaudiosink'' e ``autovideosink'' utilizam os \en{sinks}
concretos ``alsasink'' e ``xvimagesink'', os mesmos que usamos nos exemplos
anteriores.  Mas em outro ambiente essa escolha pode variar.
A Figura~\ref{fig:pipe-gen} apresenta a estrutura do \en{pipeline} criado
pelo comando anterior.

Além de criar \en{pipelines} para reprodução de áudio e vídeo, podemos usar
o \en{gst-launch} criar \en{pipelines} que transcodificam

Veja a página de manual do \en{gst-launch} (``\C{man gst-launch}'') para
mais detalhes sobre as opções suportadas, sintaxe dos argumentos (que é
similar à da Bourne Shell), exemplos de uso, etc.
