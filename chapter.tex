...
janela aparece na tela e vídeo é reproduzido até o final.
(A Figura~\ref{fig:bunny} apresenta um quadro do vídeo em questão.)  Antes
de rodar o ``Olá mundo'', porém, é preciso compilá-lo.
...
compilador~C o caminho do diretório que contém os cabeçalhos (.h) da GLib e
do GStreamer.  Além disso, precisamos informar ao \en{linker} o caminho e o
...
\en{link}-edição apropriadas dos módulos listados, no caso, ``glib-2.0'' e
...
executa o comando \en{pkg-config} e substitui o comando entre crases pelo
...
minicurso.  Na listagem, variável \C{PROGRAMS}, linha~1, contém o nome dos
...
\en{link}-edição necessárias.  O restante do arquivo, linhas~5--8, define os
alvos ``all'' e ``clean''.\footnote{Na linha~7, o primeiro caractere é um
  TAB.}  Ou seja, tendo escrito o \en{makefile}, para construir o programa
basta executar o comando ``\C{make}'' (ou ``\C{make all}'') e para remover
os arquivos gerados pelo compilador basta executar ``\C{make clean}''.
...
\en{pipeline} que lê um arquivo Ogg do disco e reproduz o seu conteúdo, ou
seja, um \en{pipeline} semelhante ao da Figura~\ref{fig:pipe-tipico} da
Seção~\ref{sec:intro}.  Antes disso, porém, vejamos um exemplo mais simples.

A Listagem~\ref{lst:mp3} apresenta um programa que reproduz um arquivo de
áudio~MP3~\cite{mp3}.  Para tal o programa monta um \en{pipeline} com três
elementos: ``filesrc'', ``mad'' e ``alsasink''.  Como vimos, no GStreamer o
próprio \en{pipeline} é um elemento---um elemento sem \en{pads} que contém
outros elementos.  Na listagem, a chamada da linha~18 aloca o elemento
\en{pipeline}, que inicialmente está vazio, e as chamadas das linhas~19--21,
alocam os elementos ``filesrc'', ``mad'' e ``alasink''.  (As chamadas
anteriores, linhas~12--16, simplesmente inicializam o GStreamer e testam se
o nome do arquivo~MP3 está em \C{argv[1]}.)
...
\clearpage
Discutimos os elementos ``filesrc'' e ``alsasink'' na Seção~\ref{sec:intro}.
O primeiro lê um arquivo do disco e gera um fluxo de bytes correspondente; o
segundo lê um fluxo de amostras de áudio PCM e as reproduz no dispositivo de
saída de áudio.  Já o elemento ``mad'' é um processador, mais precisamente,
um decodificador.  Ele lê da sua \en{source pad} um fluxo de bytes
codificado no formato~MP3, decodifica-o via MAD~\cite{mad} (biblioteca para
decodificação de áudio MP3) e escreve na sua \en{sink pad} o fluxo de áudio
PCM resultante.  Apesar de os três elementos, ``filesrc'', ``alsasink'' e
``mad'', fazerem parte do GStreamer, na prática, eles pertencem a pacotes
diferentes, distribuídos separadamente.
...
implementação é também considerada de boa qualidade mas com licenças
problemáticas; e (5)~\en{gst-plugins-bad}, contendo elementos de qualidade
inferior.\footnote{Essa terminologia é inspirada no faroeste clássico ``The
...
``mad'' do pacote \en{gst-plugins-ugly}.  (Você pode descobrir os elementos
e \en{plugins} disponíveis na sua instalação através do comando
``\C{gst-inspect}''---voltaremos a falar desse comando no final da seção.)

De volta à Listagem~\ref{lst:mp3}, a chamada \C{g_assert} na linha~22
...
e ``alsasink'' foram de fato alocados e, caso contrário, aborta o programa.
Um error comum é tentar instanciar um elemento de um \en{plugin} que não
está instalado.  Na listagem, se isso acontecer a chamada
\C{gst_element_factory_make} retornará~\C{NULL} e a asserção irá falhar.

Continuando, a chamada seguinte, linha~24, adiciona ao elemento ``pipeline''
os elementos ``filesrc'', ``mad'', ``alsasink''.  A função
\C{gst_bin_add_many} recebe um elemento \en{bin} (\C{GstBin}) e uma lista de
elementos (\C{GstElement}) terminada por \C{NULL}, e adiciona cada elemento
da lista ao \en{bin}.  Note que antes da chamada, precisamos usar a macro
\C{GST_BIN} para fazer o \en{cast} da variável \C{pipeline} para o tipo
\C{GstBin}.

A chamada~\C{gst_element_link_many}, linha~25, conecta os elementos
...
A função \C{gst_element_link_many} recebe uma lista de elementos terminada
por \C{NULL} e conecta-os em série apenas se todos possuem o mesmo elemento
pai, ou seja, estão contidos no mesmo \en{bin}.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \coordinate (A) at ($(filesrc.north west)+(-.75\odim,.75\odim)$);
    \coordinate (B) at ($(alsasink.south east)+(.75\odim,-.75\odim)$);
    \path [bin] (A) rectangle node [binlabel] {pipeline} (B);
    \draw [->, arrow] (filesrc) -- (mad);
    \draw [->, arrow] (mad) -- (alsasink);
  \end{tikzpicture}
  \caption{\en{Pipeline} GStreamer que reproduz um arquivo de áudio~MP3.}
  \label{fig:pipe-mp3}
\end{figure}

% O programa ``Olá mundo'' anterior utiliza o elemento ``playbin'' para
% tocar um vídeo no GStreamer.

% O exemplo utiliza a API de alto-nível~\texttt{GstPlayer}, atualmente a
% forma mais simples de se reproduzir um áudio ou vídeo no GStreamer.

% Apesar da API~\texttt{GstPlayer} simplificar a programação de reprodutores
% de mídia, ela é inflexível e limita os possíveis usos do framework.  Dessa
% forma, é importante que os participantes do minicurso conheçam a API
% básica do GStreamer e seu modo de operação.

% No Gstreamer, todo elemento incluindo o \en{pipeline} possui um estado,
% que pode ser nulo (\en{null}), pronto (\en{ready}), pausado (\en{paused})
% ou tocando (\en{playing}).  No estado inicial, \en{null}, o elemento não
% possui recursos alocados.  No estado \en{ready}, o elemento aloca recursos
% globais que não dependem do conteúdo a ser processado.  No estado
% \en{paused}, o elemento aloca recursos que dependem do conteúdo a ser
...
% (estado operante) o elemento tem que passar primeiro pelo estados
% \en{ready} e \en{paused}, nessa ordem.  De forma análoga, para sair do
% estado \en{playing} e voltar ao estado inicial \en{null}, o elemento tem
% que passar pelos estados \en{paused} e \en{ready}.
% A Figura~\ref{fig:estados-elt} apresenta máquina de estados de um elemento
% GStreamer, ou seja, os estados e transições possíveis.

% \begin{figure}[H]
%   \centering
%   \begin{tikzpicture}[node distance=.45\wdim+\odim]
%     \node [state] (NULL) {null};
...
%  LocalWords:  pkg-config link cflags libs flags glib gstreamer shell
