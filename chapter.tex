...
nomes dos módulos \en{pkg-config} necessárias.  E as variáveis \C{CFLAGS} e
\C{LDFLAGS}, linhas~3 e~4, contém as \en{flags} de compilação e
\en{link}-edição necessários.  O restante do arquivo, linhas~5--8, define os
alvos ``all'' e ``clean''.\footnote{Na linha~7, o primeiro caractere é um
  TAB.}  Ou seja, para construir o programa, basta executar o comando
``\C{make}'' (ou ``\C{make all}''), e para remover os arquivos gerados pelo
compilador basta executar ``\C{make clean}''.  (Veja~\cite{Gough-B-J-2005}
Para mais informações sobre como compilar programas e escrever
\en{makefiles} no GNU/Linux.)
...
Tocar um vídeo no GStreamer usando o elemento ``playbin'' é trivial.

O programa ``Olá mundo'' anterior utiliza o elemento ``playbin'' para tocar
um vídeo no GStreamer.

O exemplo utiliza a API de alto-nível~\texttt{GstPlayer}, atualmente a forma
mais simples de se reproduzir um áudio ou vídeo no GStreamer.
...
forma, é importante que os participantes do minicurso conheçam a API básica
do GStreamer e seu modo de operação.
