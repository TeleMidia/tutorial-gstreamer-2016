...
apenas um \en{alias} para a função \C{g_object_unref}, que libera uma
...
Se tudo correr bem, ao executar o programa anterior uma nova janela aparece
na tela e vídeo é reproduzido até o final.  A Figura~\ref{fig:bunny}
apresenta um quadro do vídeo em questão.  Antes de rodar o ``Olá mundo'',
porém, é preciso compilá-lo.
...
entre o apóstrofos.  Ou seja, antes de avaliar o comando mais externo,
\en{cc}, que é a chamada do compilador~C, o interpretador de comandos
(\en{shell}) executa o comando \en{pkg-config} e substitui o que aparece
entre apóstrofos pelo resultado dessa execução.

No nosso ambiente, o comando anterior avalia para:
\begin{lstlisting}[style=command]
@\$@ cc -pthread\
...
programa.  A Listagem~\ref{lst:makefile} apresenta o \en{makefile} que
usamos para construir os programas deste minicurso.  Usando esse
\en{makefile} sempre que for preciso recompilar o programa basta rodar o
comando \en{make} no mesmo diretório onde está o arquivo de \en{makefile}
(``Makefile'').  E sempre que quisermos remover os arquivos gerados pelo
compilador basta rodar ``\C{make clean}''.  Para mais informações sobre como
compilar programas no GNU/Linux, veja~\cite{Gough-B-J-2005}.
