...
da Figura~\ref{fig:pipeline-tipico}, para manter a taxa de reprodução
necessária, os \en{sinks} ``alsasink'' e ``xvimagesink'' armazenam as
amostras recebidas num \en{buffer} interno, e exibem-nas (consomem-nas)
apenas no momento adequado.  Já os outros elementos operam livres, isto é,
consomem e produzem dados em taxas arbitrárias.  Se durante a execução os
elementos que precedem os \en{sinks} operarem rápido o bastante, os
\en{buffers} dos \en{sinks} nunca ficarão vazios e todas as amostras serão
exibidas no momento correto.  Mas esse nem sempre é o caso.

Caso os elementos fluxo acima operem abaixo da taxa de consumo dos
\en{sinks}, pode ser que o \en{buffer} interno de um dos \en{sinks} esvazie.
Nesse caso, se chegar o momento de exibir uma amostra o \en{sink}
simplesmente não tem o que exibir, o que causa interrupções ou saltos na
reprodução---as amostras ``atrasadas'' quando chegarem serão descartadas.
Há ainda o problema inverso, caso os elementos fluxo acima operem muito
acima da taxa de consumo dos \en{sinks}, pode ser que o \en{buffer} interno
estoure e amostras sejam perdidas.

Para evitar o esvaziamento ou estouro de \en{buffers} nos sinks, ou mesmo
para limitar o uso de CPU, os \en{sinks}---na verdade, qualquer elemento que
sincronize amostras---enviam eventos de QoS (\en{quality of service}) aos
elementos fluxo acima.
...
%  LocalWords:  player players fast-foward rewind plugins demultiplexa
%  LocalWords:  demultiplexa-o demultiplexador demultiplexando
