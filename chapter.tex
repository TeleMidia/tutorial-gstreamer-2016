...
taxas são~44100Hz para amostras de áudio e~30Hz para amostras de vídeo: a
cada~22{,}67\textmu{s} uma nova amostra de áudio deve ser enviada ao
...
consumo dos \en{sinks}, pode ser que a capacidade da fila interna seja
...
\en{sinks} normalmente enviam fluxo acima eventos de QoS (\en{quality of
  service}) que indicam o quão atrasada ou adiantada está cada amostra que
...
A função \C{gst_element_factory_make} aloca e retorna um novo elemento
...
referência para um \C{GObject}.  No GStreamer, a maioria dos tipos
``objeto'' não herdam diretamente de \C{GObject}, mas sim de \C{GstObject}
que por sua vez herda de \C{GObject}.
...
mais fácil de se fazer isso é através ferramenta \en{pkg-config}.  Por
exemplo:
...

\section{Construindo o \en{pipeline} anterior a partir de elementos básicos}
\label{sec:dissec}

Na Seção~\ref{sec:ola} utilizamos o elemento ``playbin'' para tocar um vídeo
Ogg no GStreamer.  Nesta seção, faremos a mesma aplicação mas dessa vez sem
usar o elemento ``playbin''.  Usando apenas elementos básicos (\en{sources},
demultiplexadores, decodificadores e \en{sinks}) vamos construir um
\en{pipeline} semelhante ao da Figura~\ref{fig:pipe-tipico},
Seção~\ref{sec:intro}, que lê um arquivo Ogg do disco, demultiplexa-o, e
decodifica e reproduz os fluxos de áudio e vídeo resultantes.  Antes disso,
porém, vejamos um exemplo mais simples.
...
e ``alsasink'' foram de fato alocados.  Um error comum é tentar instanciar
um elemento de um \en{plugin} que não está instalado.  Na listagem, se isso
acontecer a chamada \C{gst_element_factory_make} retornará~\C{NULL} e a
asserção falhará, abortando o programa.

Continuando, a chamada da linha~21 adiciona três filhos ao \en{pipeline}:
``filesrc'', ``mad'' e ``alsasink''.  A função \C{gst_bin_add_many} recebe
um \en{bin} (\C{GstBin}) e uma sequência de elementos (\C{GstElement})
terminada por \C{NULL}, e adiciona os elementos da sequência ao \en{bin}.
Note que antes da chamada, precisamos usar a macro \C{GST_BIN} para
converter a variável \C{pipeline} para o tipo da sua superclasse~\C{GstBin}.
...
filhos do mesmo \en{bin}).  A Figura~\ref{fig:pipe-mp3} apresenta a
...
Seção~\ref{sec:ola}, faz com que os seus filhos ``filesrc'', ``mad'' e
...
\en{ready} e \en{paused}, nessa ordem.  De forma análoga, para sair do
estado \en{playing} e voltar ao estado inicial \en{null}, o elemento tem que
passar pelos estados \en{paused} e \en{ready}.
...
do \en{pipeline}, na verdade de \en{bins} em geral, são propagadas nos
elementos filhos, e a direção da propagação é sempre dos \en{sinks} para os
\en{sources}---o que garante que dados não sejam gerados antes que os
elementos fluxo abaixo estejam prontos para recebê-los.  Por exemplo, na
Listagem~\ref{lst:mp3}, se bem sucedida, a chamada \C{gst_element_set_state}
(linha~25) implica na seguinte sequência transições (sempre dos \en{sinks}
para os \en{sources}): (1)~os elementos filhos ``alsasink'', ``mad'' e
``filesrc'' transicionam de \en{null} para \en{ready} e, em seguida, o
\en{pipeline} transiciona para \en{ready}; (2)~os filhos transicionam de
\en{ready} para \en{paused} e, em seguida, o \en{pipeline} transiciona para
\en{paused}; e~(3) os filhos transicionam de~\en{paused} para \en{playing}
e, finalmente, o \en{pipeline} transiciona para \en{playing}.  Essa última
transição completa a chamada e devolve o controle à função \C{main}.  Nesse
ponto, a \en{thread} da aplicação está prestes a executar a asserção da
linha~26, que aborta o programa em caso de falha da chamada anterior, e as
\en{streaming threads} já começaram a operar o \en{pipeline}, ou seja, os
...
\begin{figure}[b]
  ...
  \end{tikzpicture}
  \caption{Máquina de estados de elemento (\C{GstElement}).}
  \label{fig:estados-elt}
\end{figure}

A chamada seguinte, linha~28, obtém uma referência para o barramento do
\en{pipeline} e a próxima, linha~29, e bloqueia a \en{thread} da aplicação
...
aplicada a um \en{bin} libera não apenas \en{bin} mas também os seus filhos,
ou seja, nesse ponto os elementos alocados nas linhas~15--28 são liberados.
...
\en{pipeline} semelhante ao apresentado na Seção~\ref{sec:intro} que usa
apenas elementos básicos para tocar um vídeo~Ogg.  Há uma diferença
importante entre o \en{pipeline} Ogg (Figura~\ref{fig:pipe-tipico}) que
pretendemos construir e o \en{pipeline} MP3 (Figura~\ref{fig:pipe-mp3}) que
construímos anteriormente.  Enquanto no \en{pipeline} MP3 o número de
\en{pads} de todos os elementos é fixo (conhecido em tempo de compilação),
no \en{pipeline} Ogg esse número é variável.  Em particular, no
\en{pipeline} Ogg o número de \en{sink pads} do elemento ``oggdemux''
depende do número de subfluxos multiplexados fluxo Ogg de entrada.  Por
...
\en{sink pads}, uma para o subfluxo Vorbis resultante, que deverá ser
conectada ao elemento ``vorbisdec'', e outra para o subfluxo Theora
resultante, que deverá ser conectada ao elemento ``theoradec''.  Dessa
forma, o número de subfluxos multiplexados no fluxo Ogg de entrada determina
o número de \en{sink pads} no demultiplexador.  \en{Pads} desse tipo,
criadas sob demanda pelo elemento, são chamadas de \en{sometimes pads}.

No GStreamer, toda \en{pad} é instanciada de acordo um \en{template} que
indica a sua direção, capacidade e disponibilidade.  A direção (\en{source}
ou \en{sink}) determina se a \en{pad} produz ou consome dados.
...
são criadas pelo próprio elemento apenas sob condições específicas que
normalmente envolvem o conteúdo processado; e~(3)\en{request pads} são
criada apenas sob requisição explícita da aplicação.

No \en{pipeline} MP3 da Figura~\ref{fig:pipe-mp3} as \en{pads} de todos os
elementos possuem disponibilidade \en{always}.  Isso significa que podemos
construir o \en{pipeline} inteiro estaticamente, conectando todos os seus
elementos antes de iniciá-lo.  Já no \en{pipeline} Ogg da
Figura~\ref{fig:pipe-tipico} essa abordagem não é possível.  As \en{sink
  pads} do elemento ``oggdemux'' possuem disponibilidade \en{sometimes} e só
serão criadas quando o elemento decodificar o cabeçalho do fluxo de entrada
(isto é, transicionar para o estado \en{paused}).  Nesse caso, precisamos
usar uma abordagem incremental com dois passos.  Primeiro, precisamos montar
e iniciar um \en{pipeline} contendo apenas os elementos ``filesrc'' e
``oggdemux'' conectados.  Em seguida, precisamos esperar o elemento
``oggdemux'' notificar a criação de cada \en{sink pad} para então adicionar
e conectar os elementos restantes de acordo com as capacidades da \en{pad}
criada.  No GStreamer, esse tipo de notificação assíncrona disparada por
elementos é chamada de sinal.  Todo sinal possui um nome (``pad-added'',
nesse caso) e pode ser capturado via um mecanismo de registro de
\en{callbacks}.  Ou seja, para capturar um sinal a aplicação registra no
elemento uma função \en{callback} que é chamada sempre que o sinal é
disparado.
...
``audioconvert'' converte as amostras produzidas pelo ``vorbisdec'' para um
formato que o ``alsasink'' aceita.  Na prática, não é possível conectar o
elemento ``vorbisdec'' diretamente ao ``alsasink''---as \en{caps} da
\en{source pad} do ``vorbisdec'' não são compatíveis com as \en{caps} do
\en{sink pad} do ``alsasink'', isto é, a interseção das \en{caps} dessas
...
Já o elemento ``queue'' entre os elementos ``oggdemux'' e ``theoradec''
serve para
...
Vejamos o propósito de cada linha da Listagem~\ref{lst:ogg}.
...
%  LocalWords:  threads streaming bus timed filtered warnings CLOCK NONE
...
%  LocalWords:  PROGRAMS LDFLAGS TAB makefiles mad gst-plugins-base LGPL
...
%  LocalWords:  main callbacks callback sometimes template caps always
%  LocalWords:  request pad-added audioconvert queue
