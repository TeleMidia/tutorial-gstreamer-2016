...
\en{thread} da aplicação.  Consequentemente, caso as \en{threads}
compartilhem dados, para evitar condições de corrida, é necessário
...
fazer isso porque as chamadas \C{gst_element_$\ast$} e~\C{gst_bin_$\ast$}
que usamos na \en{callback} para manipular o elemento ``pipeline'' (único
dado compartilhado) já são protegidas internamente por \en{mutexes}.  Além
disso, no intervalo em que as \en{threads} da \en{callback} e da aplicação
estão de fato concorrendo, linhas~77--80, a \en{thread} da aplicação não
modifica o \en{pipeline}.
...
O comando \en{gst-launch} é particularmente útil para depurar \en{pipelines}
ou para testar a sua viabilidade antes de implementá-los em~C.  Por exemplo,
usando o \en{gst-launch} podemos construir rapidamente uma versão genérica
...
transcodificam e transmitem fluxos de mídia na rede.  Para informações sobre
a sintaxe do comando (que é inspirada na sintaxe de \en{pipelines} da Bourne
Shell), opções suportadas e exemplos de uso, veja a página de manual do
\en{gst-launch} (``\C{man gst-launch}'').

Outro comando interessante instalado pelo pacote \en{gstreamer} é o comando
\en{gst-inspect}, que permite inspecionar uma instalação
GStreamer.\footnote{Em algumas instalações o nome do comando é
  \en{gst-inspect-1.0}.}

% Quando executado sem parâmetros ele lista os \en{plugins} e elementos
% disponíveis na instalação--por exemplo, na nossa instalação há~223
% \en{plugins} e~1302 elementos.  Quando chamado com o nome de um \en{plugin}
% ele lista as informações do \en{plugin}.
