...
chamadas com prefixo ``\C{g_}'' ou ``\C{G_}'' e referem-se à funções ou
macros da GLib, e as chamadas com prefixo ``\C{gst_}'' ou ``\C{GST_}'' e as
declarações com prefixo ``\C{Gst}'' referem-se à funções, macros ou tipos do
GStreamer.

\clearpage
A chamada~\C{gst_init} na linha~11 inicializa o GStreamer e trata os
...
\en{pads}; tais elementos contêiner são chamados de ``\en{bins}''.  A função
\C{gst_element_factory_make} aloca e retora um novo elemento
...
A chamada \C{g_assert_nonnull}, linha~14, é uma asserção que garante que a
...
A chamada \C{g_object_set} na linha~18 atribui a URI criada anteriormente à
...
A chamada seguinte, linha~19, libera a \en{string} alocada na linha~16.
Nesse ponto a \en{string} já não é mais necessária pois foi copiada pela
chamada \C{g_object_set} anterior.

A chamada \C{gst_element_set_state}, linha~21, inicia o \en{pipeline} da
aplicação, isto é, transiciona elemento ``playbin'' para o estado
\C{GST_STATE_PLAYING}.  No Gstreamer, todo elemento incluindo o
...
% Um elemento ``playbin'', tipo \C{GstPlayBin}, é uma especialização de um
% elemento ``pipeline'', tipo \C{GstPipeline}, que por sua vez é uma
% especialização de um ``bin'', tipo \C{GstBin}.  Esse último herda de
% \C{GstElement}, o tipo comum a todos os elementos, que eventualmente herda
% de \C{GObject} o tipo raiz da hierarquia de objetos da GObject.
...
%  LocalWords:  upstream EOS end-of-stream flush LocalWords
