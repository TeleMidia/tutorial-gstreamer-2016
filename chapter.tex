...
compilador~C o caminho do diretório que contém os cabeçalhos da GLib e do
...
alvos ``all'' e ``clean''.  (Na linha~7, o primeiro caractere é um
...
que contém outros elementos.  Na listagem, a chamada da linha~18 aloca o
elemento \en{pipeline}, que inicialmente está vazio, e as chamadas das
linhas~19--21, alocam os elementos ``filesrc'', ``mad'' e ``alsasink''.  (As
chamadas anteriores, linhas~12--16, simplesmente inicializam o GStreamer e
testam se o programa foi chamado corretamente, isto é, se o caminho para o
arquivo~MP3 está em \C{argv[1]}.)
...
Continuando, a chamada da linha~24 adiciona três filhos ao elemento
``pipeline'': os elementos ``filesrc'', ``mad'' e ``alsasink''.  A função
\C{gst_bin_add_many} recebe um \en{bin} (\C{GstBin}) e uma sequência de
elementos (\C{GstElement}) terminada por \C{NULL}, e adiciona cada elemento
da sequência ao \en{bin}.  Note que antes da chamada, precisamos usar a
macro \C{GST_BIN} para converter a variável \C{pipeline} para o tipo da sua
superclasse~\C{GstBin}.
...
A chamada~\C{g_object_set}, linha~25, atribui o caminho do arquivo MP3 a ser
...
A chamada \C{gst_element_set_state}, linha~28, transiciona o ``pipeline'' do
...
GStreamer (isto é, os estados e as transições possíveis).  Mudanças de
estado do \en{pipeline}, na verdade de \en{bins} em geral, são propagadas
nos elementos filhos, e a direção da propagação é sempre dos \en{sinks} para
os \en{sources}, o que garante que dados não sejam gerados antes que os
...
(linha~28) implica na seguinte sequência transições (sempre dos \en{sinks}
...
transição completa a chamada que devolve o controle à função \en{main}.
Nesse ponto, a \en{thread} da aplicação está prestes a executar a asserção
da linha~29, que aborta o programa em caso de falha da chamada anterior, e
...
até que erro aconteça ou até que o arquivo de áudio seja reproduzido
completamente (EOS).

Assim que a chamada \C{gst_bus_timed_pop_filtered} retorna, a \en{thread} da
aplicação libera as referências do bus e da mensagem retornada
(linhas~34--35), transiciona o \en{pipeline} (linha~36) de volta para o
estado \en{null} e libera a sua referência (linha~37).  Nesse caso, a
chamada \C{gst_element_set_state} (linha~36) também propaga a transição para
os elementos filhos, só que agora as transições são no sentido
contrário---de \en{playing} para \en{paused}, \en{ready} e \en{null}.
Finalmente, observe que a chamada \C{gst_object_unref} (linha~37) quando
aplicada a um \en{bin} libera não apenas \en{bin} mas também os seus filhos,
ou seja, nesse ponto os elementos alocados nas linhas~18--21 são liberados.
