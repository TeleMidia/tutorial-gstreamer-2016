...
dos \en{sinks}, pode ser que a fila interna de um dos \en{sinks} fique vazia e,
...
Tais elementos contêiner são chamados de ``\en{\rghl{bins}{Não deveria ter sido
discutido na seção anterior?}''}.  A função \C{gst_element_factory_make} aloca
...
referência para um \C{GObject}.  No GStreamer, a maioria dos \rghl{tipos
``objeto''}{Entendi o que é. Mas não tem como falarmos isso de uma forma
melhor?} não herdam diretamente de \C{GObject}, mas sim, de \C{GstObject} que,
por sua vez, herda de \C{GObject}.
...
Continuando, a chamada da \C{gst_bin_add_many}, linha~21, adiciona três filhos
...
\rg{Esses dois próximos parágrafos também são candidatos a sair se precisarmos
    de espaço.}
