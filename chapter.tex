...
\begin{resumo}
  ...
  filtros---elementos que manipulam as amostras de áudio e vídeo.
...
por onde fluem os dados--e.g. as amostras de áudio e vídeo e dados de controle.
...
API~C~\cite{Kernighan-B-W-1988} básica (de baixo nível) do GStreamer~1.10, sua
...
entender o modelo de computação no qual o \en{framework} se baseia, conhecido
como \en{dataflow}.  No restante dessa seção apresentamos esse modelo e
discutimos a forma como ele é instanciado no GStreamer.
...
processadores (``oggdemux'', ``vorbisdec'' e ``theoradec'') e dois consumidores
...
através da sua \en{source pad} para a \en{sink pad} do elemento ``oggdemux''.
No diagrama essa conexão entre as \en{pads} é representada pela aresta entre os
elementos ``filesrc'' e ``oggdemux''.  Similarmente, as demais arestas denotam
conexões entre \en{source pads} e \en{sink pads}.
...
elemento---um elemento que contém outros elementos mas não possui \en{pads}.
Tais elementos contêiner são chamados de ``\en{bins}''.  A função
...
Note que antes da chamada precisamos usar a macro \C{GST_BIN} para converter
a variável \C{pipeline} para o tipo da sua superclasse~\C{GstBin}.
...
Para chegar do estado \en{null} (inicial) ao estado \en{playing} (operante)
...
máquina de estados de um elemento GStreamer (isto é, seus estados e
transições possíveis).  Mudanças de estado do \en{pipeline}, na verdade de
\en{bins} em geral, são propagadas nos elementos filhos, e a direção da
propagação é sempre dos \en{sinks} para os \en{sources}---dessa forma dados
não são gerados antes que os elementos fluxo abaixo estejam prontos para
recebê-los.  Por exemplo, na Listagem~\ref{lst:mp3}, se bem sucedida, a
...
para \en{playing}.  Essa última transição completa a chamada e devolve o
controle à função \C{main}.  Nesse ponto, a \en{thread} da aplicação está
prestes a executar a asserção da linha~26, que aborta o programa em caso de
falha da chamada anterior, e as \en{streaming threads} já começaram a operar
o \en{pipeline}, ou seja, os elementos ``filesrc'', ``mad'' e ``alsasink''
já estão produzindo, processando e consumindo dados.
...
\en{paused} seus elementos (nesse ponto, ``filesrc'' e ``oggdemux'') já
...
Um equalizador é um processador de áudio que permite modificar o ganho dos
graves, médios e agudos das amostras de um fluxo de áudio descomprimido.  Os
três equalizadores instalados pelo \en{plugin} ``equalizer'' anterior
possuem a mesma estrutura.  Os três têm uma \en{source pad} que consome
áudio descomprimido e uma \en{sink pad} que produz audio descomprimido, e
todos eles processam uma amostra por vez: leem uma amostra da sua \en{source
  pad}, modificam-na acordo com os valores de ganho especificados em suas
propriedades e escrevem a amostra resultante na sua \en{sink pad}.
...
amostras descomprimidas.  A transformação em si é normalmente ortogonal,
isto é, depende apenas do conteúdo das amostras, e altera esse conteúdo afim
de produzir efeitos sonoros ou visuais no fluxo resultante.
A Tabela~\ref{tab:filtros} apresenta alguns filtros de áudio e vídeo
disponíveis na distribuição oficial do GStreamer.
...
\begin{table}[h]
  ...
  \begin{tabular}{llll}
    ...
    &effecttv    & shagadelictv & Efeito de espiral caleidoscópica\\
    &effectv     & agingtv     & Efeito de envelhecimento\\
    &effectv     & rippletv    & Efeito de ondulação\\
...
Todo filtro possui exatamente uma \en{source pad} e uma \en{sink pad}.
...
Listagem~\ref{lst:mp3}, que reproduz um arquivo de áudio MP3.
\begin{lstlisting}[style=command]
...
% A Figura~\ref{fig:pipe-mp3-volume} apresenta a estrutura do \en{pipeline}
% criado pelo comando \en{gst-launch} anterior.
% \begin{figure}[h]
%   \centering
%   \begin{tikzpicture}
%     \node (filesrc) [element] {filesrc};
%     \node (mad) [element, right of=filesrc] {mad};
%     \node (volume) [element, right of=mad] {volume};
%     \node (alsasink) [element, right of=volume] {alasink};
%     \coordinate (A) at ($(filesrc.north west)+(-.5\odim,.5\odim)$);
%     \coordinate (B) at ($(alsasink.south east)+(.5\odim,-.5\odim)$);
%     \path [bin] (A) rectangle node [binlabel] {pipeline} (B);
%     \draw [->, arrow] (filesrc) -- (mad);
%     \draw [->, arrow] (mad) -- (volume);
%     \draw [->, arrow] (volume) -- (alsasink);
%   \end{tikzpicture}
%   \caption{\en{Pipeline} que reproduz áudio~MP3 com a metade do volume
%     original.}
%   \label{fig:pipe-mp3-volume}
% \end{figure}
...
um vídeo com um dado efeito de áudio e vídeo.  O \en{script} recebe dois
...
mathescape=false,
style=display,
caption={\en{Shell script} que reproduz um vídeo aplicando efeitos de áudio
  e vídeo.},
...
e~$\<\text{\en{video filter}}>$ (variáveis~\C{af} e~\C{vf} na listagem)
variam de acordo com o valor do segundo argumento do \en{script}.  Há cinco
possibilidades:
\begin{itemize}
\item Se o argumento é~0, pela linha~3 na listagem, o \en{script} utiliza o
  filtro de áudio ``volume'' com a propriedade ``volume'' inicializada
  com~2, e o filtro de vídeo ``coloreffects'' com a propriedade ``preset''
  inicializada com~1.  Nesse caso, as amostras de áudio serão reproduzidas
  com o dobro do volume original e as amostras de vídeo serão colorizadas
  para simular o efeito ``câmera de calor''
  (Figura~\ref{fig:filtros-screen}.b).
\item Se o argumento é~1, pela linha~4, o \en{script} utiliza o filtro de
  áudio ``pitch'' com a propriedade ``pitch'' inicializada com~.5, e o
  filtro de vídeo ``rippletv''.  Nesse caso, as amostras de áudio serão
  reproduzidas na metade do tom (frequência) original e as amostas de vídeo
  serão alteradas para simular um efeito de ondulação
  (Figura~\ref{fig:filtros-screen}.c).
\item Se o argumento é~2, pela linha~5, o \en{script} utiliza o filtro de
  áudio ``audioecho'' com as propriedades ``delay'' e ``intensity''
  inicializadas com~.5s e~1, e o filtro de vídeo ``shagadelictv''.  Nesse
  caso, as amostras de áudio serão reproduzidas com eco, e as amostras de
  vídeo simulam serão alteradas para simular o efeito espiral caleidoscópica
  (Figura~\ref{fig:filtros-screen}.d).
\item Se o argumento é~3, pela linha~6, \en{script} utiliza o filtro áudio
  ``freeverb'' com as propriedades ``room-size'' e ``level'' inicializadas
  com~1 e~1, e o filtro de vídeo ``edgetv''.  Nesse caso, as amostras de
  áudio serão alteradas para simular a reverberação numa sala ampla, e as
  amostras de vídeo serão alteradas para mostrar apenas as bordas da imagem
  de cada quadro (Figura~\ref{fig:filtros-screen}.e).
\item Finalmente, se o argumento é~4, pela linha~7, o \en{script} utiliza o
  filtro de áudio ``equalizer-3bands'' com as propriedades ``band0'' e
  ``band1'' inicializadas com~12dB e~-24dB, e o filtro de vídeo ``revtv''.
\end{itemize}
...
%  LocalWords:  templates transicioná-los
