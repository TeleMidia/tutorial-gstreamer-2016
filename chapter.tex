...
Todo filtro possui exatamente uma \en{source pad} e uma \en{sink pad}; ambas
têm disponibilidade \en{always} e escoam apenas fluxos descomprimidos
(``audio/x-raw'' ou ``video/x-raw'').  Consequentemente, num \en{pipeline}
os elementos filtros normalmente aparecem entre os decodificadores e os
\en{sinks}.  Por exemplo, o comando seguinte adiciona o filtro ``volume'' ao
\en{pipeline} da Listagem~\ref{lst:mp3}, que reproduz um arquivo de áudio
MP3.
\begin{lstlisting}[style=command]
...
(passado por referência à \en{callback} via o parâmetro~\C{data}).
...
eles constroem um \en{pipeline} que reproduz o conteúdo do inicio ao fim uma
única vez.  Nesta seção vamos adicionar os controles \en{play}, \en{pause},
\en{stop}, \en{seek}, \en{fast-forward} e \en{rewind} ao programa ``Olá
mundo'' da Listagem~\ref{lst:hello}.  Nosso objetivo é fazer com que o
programa responda interativamente aos seguintes comandos de tecla:
\begin{center}
  \begin{tabular}{cl}
...
qualquer aplicação GStreamer.  Vejamos primeiro como adicionar os controles
mais simples: \en{play}, \en{pause} e~\en{stop}.


\subsection*{\en{Play}, \en{pause} e \en{stop}}

\lstinputlisting[
style=display,
caption={Adicionando controles ao programa ``Olá mundo''
  da Listagem~\ref{lst:hello}}.
label={lst:controles},
]{src/controls.c}
