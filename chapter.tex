...
No exemplo anterior, o primeiro comando cria um \en{pipeline} contento
...
\en{pipeline} com os elementos ``filesrc'', ``mad'' e ``alsasink''
...
\en{pipeline} preliminar contento apenas os elementos ``filesrc'' e
``oggdemux'' conectados, atribui o caminho do vídeo à propriedade
``location'' do ``filesrc'' e inicia o \en{pipeline}.  Em seguida, quando o
...
viabilidade de \en{pipelines} antes de implementá-los em~C.  Por exemplo,
...
ambiente.\footnote{Em algumas instalações o nome do comando é
  \en{gst-inspect-1.0}.}  Quando executado sem parâmetros o \en{gst-inspect}
...
argumentos o \en{gst-inspect} apresenta as informações do dado \en{plugin}
ou elemento.  A Listagem~\ref{lst:equalizer} apresenta a saída do comando
``\C{gst-inspect$~$equalizer}'' lista as informações do \en{plugin}
\en{equalizer}.
...
\en{plugin} (\en{equalizer}), a sua descrição (equalizadores de áudio) e o
...
exemplo, basta rodar o comando ``\C{gst-inspect equalizer-3bands}''.
A saída nesse caso contém informações como a hierarquia de tipos do
elemento, as interfaces que ele implementa, os \en{templates} de suas
\en{pads}, as \en{pads} propriamente ditas (no caso, uma \en{source pad} com
disponibilidade \en{always} do tipo ``audio/x-raw'', e uma \en{sink pad}
também \en{always} e ``audio/x-raw'') e as propriedades do elemento.
...
%  LocalWords:  gst-launch Bourne equalizer equalizer-nbands bands
