...
\begin{frame}[fragile]{gst-inspect}
  ...
  Exemplo: \texttt{\$ gst-inspect equalizer}
...
\end{lstlisting}
  ...
  \begin{adjustbox}{max totalsize={.8\textwidth}{\textheight},center}
    \begin{tikzpicture}
      \node (uridecodebin) [element] {uridecodebin};
      \node (videotestsrc) [element, below of=uridecodebin] {videotestsrc};

      \node (compositor) [element, right of=uridecodebin] at ($(uridecodebin)!0.5!(videotestsrc)$) {compositor} ;
      \node (videofilter) [element, right of=compositor] {videofilter};
      \node (xvimagesink) [element, right of=videofilter] {xvimagesink};

      \draw [->, arrow] (uridecodebin) -- (compositor);
      \draw [->, arrow] (videotestsrc) -- (compositor);
      \draw [->, arrow] (compositor) -- (videofilter);
      \draw [->, arrow] (videofilter) -- (xvimagesink);
    \end{tikzpicture}
  \end{adjustbox}
\end{frame}
...
\end{lstlisting}
  ...
  \begin{adjustbox}{max totalsize={.8\textwidth}{\textheight},center}
    \begin{tikzpicture}
      \node (uridecodebin) [element] {uridecodebin};
      \node (audioconvert1) [element, right of=uridecodebin] {audioconvert};
      \node (audiotestsrc) [element, below of=uridecodebin] {audiotestsrc};
      \node (audioconvert2) [element, right of=audiotestsrc] {audioconvert};

      \node (compositor) [element, right of=audioconvert1] at ($(audioconvert1)!0.5!(audioconvert2)$) {compositor} ;
      \node (audioconvert3) [element, right of=compositor] {audioconvert};
      \node (alsasink) [element, right of=audioconvert3] {alsasink};

      \draw [->, arrow] (uridecodebin) -- (audioconvert1);
      \draw [->, arrow] (audiotestsrc) -- (audioconvert2);
      \draw [->, arrow] (audioconvert1) -- (compositor);
      \draw [->, arrow] (audioconvert2) -- (compositor);
      \draw [->, arrow] (compositor) -- (audioconvert3);
      \draw [->, arrow] (audioconvert3) -- (alsasink);
    \end{tikzpicture}
  \end{adjustbox}
\end{frame}
...
\begin{frame}[c]{Considerações Finais}
  \begin{itemize}
    \item Sincronização de relógios entre \en{pipelines}
      \begin{itemize}
        \item \C{GstNetTimeProvider} + \C{GstNetClientClock}
      \end{itemize}
    \item Sincronização de relógios com um servidor NTP 
      (\en{Network Time Protocol})
      \begin{itemize}
        \item \C{GstNtpClock}
      \end{itemize}
    \item Sincronização de relógios com um servidor PTP 
      (\en{Precision Time Protocol})
      \begin{itemize}
        \item \C{GstPtpClock}
      \end{itemize}
    \item \en{Bindings} para outras linguagens/plataformas
      \begin{itemize}
        \item Lua, Python, Java, C++, Qt, Android, Vala, Ryby, 
          Haskell, \ldots.
      \end{itemize}
  \end{itemize}
\end{frame}
